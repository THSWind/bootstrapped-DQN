...
"""
Contains environment instantiation, pixel/etc. definitions, reset,
pos from unprocessed state, obj/states from pixels, preprocesing
https://github.com/uber-research/go-explore/blob/master/goexplore_py/montezuma_env.py
...
class MyMontezuma:
    def __init__(self, check_death: bool=True, unprocessed_state: bool=False,
                 score_objects: bool=False, x_repeat=2,
                 objects_from_pixels=True, objects_remember_rooms=False,
                 only_keys=False):
    ...
    def get_objects_from_pixels(self, unprocessed_state, room, old_objects):
        ...
        # Note: this "ground truth" logic is excluded because the RAM and the
        # frames are not updated at the same time, so that the asserts would
        # fail even in normal cases. Commented all related code and not just
        # the asserts so that we don't waste time making checks that are
        # impossible.
        ...
        if self.objects_remember_rooms:
            ...
            # TODO: bring back these asserts. Unfortunately the ram and the
            # frames aren't updated at he same time so these would normally
            # fail :(
            ...
            # TODO: bring back these asserts. Unfortunately the ram and the
            # frames aren't updated at the same time so these would normally
            # fail :(
            # assert all(e == 0 for e in ground_truth.values())
            return cur_object
    ...
    def restore(self, data):
        (full_state, state, score, steps, pos, room_time, ram_death_state,
         self.score_objects, self.cur_lives) = data
    ...
    def is_transition_screen(self, unprocessed_state):
        ...
        # The screen is a transition screen if it is all black or if its color
        # is made up only of black and (0, 28, 136), which is a color seen in
        # the transition screens between two levels.
        return (
    ...
    def is_pixel_death(self, unprocessed_state, face_pixels):
        # There are no face pixels and yet we are not in a transition screen.
        # We must be dead!
        if len(face_pixels) == 0:
            # All of the screen except the bottom is black: this is not a
            # death but a room transition. Ignore.
    ...
    def render_with_known(self, known_positions, resolution, show=True,
                          filename=None, combine_val=max,
